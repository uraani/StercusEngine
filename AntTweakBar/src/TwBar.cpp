    ...
    case TW_TYPE_CHAR:
        {
            ...
            if( Val!=0 )
            {
                ...
                if( m_Val.m_Char.m_Hexa )
                    SDL_snprintf(Tmp, 1024, "%c (0x%.2X)", Val, d);
                else
					SDL_snprintf(Tmp, 1024, "%c (%d)", Val, d);
                *_Str = Tmp;
            }
            else
            {
    ...
    case TW_TYPE_INT8:
        {
            ...
            if( m_Val.m_Int8.m_Hexa )
				SDL_snprintf(Tmp, 1024, "0x%.2X", d&0xff);
            else
				SDL_snprintf(Tmp, 1024, "%d", d);
            *_Str = Tmp;
        }
        break;
    case TW_TYPE_UINT8:
        {
            ...
            if( m_Val.m_UInt8.m_Hexa )
				SDL_snprintf(Tmp, 1024, "0x%.2X", d);
            else        
				SDL_snprintf(Tmp, 1024, "%u", d);
            *_Str = Tmp;
        }
        break;
    case TW_TYPE_INT16:
        {
            ...
            if( m_Val.m_Int16.m_Hexa )
				SDL_snprintf(Tmp, 1024, "0x%.4X", d&0xffff);
            else
				SDL_snprintf(Tmp, 1024, "%d", d);
            *_Str = Tmp;
        }
    ...
    case TW_TYPE_INT32:
        {
            ...
            if( m_Val.m_Int32.m_Hexa )
				SDL_snprintf(Tmp, 1024, "0x%.8X", Val);
            else
				SDL_snprintf(Tmp, 1024, "%d", Val);
            *_Str = Tmp;
        }
        break;
    case TW_TYPE_UINT32:
    ...
    case TW_TYPE_FLOAT:
        {
            ...
            if( m_Val.m_Float32.m_Precision<0 )
				SDL_snprintf(Tmp, 1024, "%g", Val);
            else
            {
                char Fmt[64];
				SDL_snprintf(Fmt, 64, "%%.%df", (int)m_Val.m_Float32.m_Precision);
				SDL_snprintf(Tmp, 1024, Fmt, Val);
            }
            *_Str = Tmp;
        }
        break;  
    case TW_TYPE_DOUBLE:
        {
            ...
            {
                char Fmt[128];
				SDL_snprintf(Fmt, 128, "%%.%dlf", (int)m_Val.m_Float64.m_Precision);
				SDL_snprintf(Tmp, 1024, Fmt, Val);
            }
            *_Str = Tmp;
        }
...
int CTwVarAtom::SetAttrib(int _AttribID, const char *_Value, TwBar *_Bar, struct CTwVarGroup *_VarParent, int _VarIndex)
{
    ...
    case VA_STEP:
        if( _Value && strlen(_Value)>0 )
        {
            ...
            if( Fmt!=NULL && Ptr!=NULL && SDL_sscanf(_Value, Fmt, Ptr)==1 )
            {
    ...
    case VA_PRECISION:
        ...
        {
            int Precision = 0;
            if(SDL_sscanf(_Value, "%d", &Precision)==1 && Precision>=-1 && Precision<=12 )
            {
            ...
            if( !( m_Type==TW_TYPE_BUTTON || IsCustom() ) ) // || (m_Type>=TW_TYPE_CUSTOM_BASE && m_Type<TW_TYPE_CUSTOM_BASE+(int)g_TwMgr->m_Customs.size()) ) )
            {
                ...
                else if( IsCSStringType(m_Type) )
                {
                    ...
                    if( n>0 )
                    {
                        ...
                        {
                            if( n>1 )
								SDL_strlcpy((char *)m_Ptr, str.c_str(), n-1);
                            ((char *)m_Ptr)[n-1] = '\0';
                            _Bar->NotUpToDate();
                            return 1;
                        }
...
int CTwVarGroup::SetAttrib(int _AttribID, const char *_Value, TwBar *_Bar, struct CTwVarGroup *_VarParent, int _VarIndex)
{
    ...
    case VG_TYPEID:
        {
            ...
            if( _Value!=NULL && SDL_sscanf(_Value, "%d", &type)==1 )
            {
    ...
    case VG_VALPTR:
        {
            ...
            if( _Value!=NULL && SDL_sscanf(_Value, "%p", &structValuePtr)==1 )
            {
    ...
    case VG_ARROW:
        if( m_SummaryCallback==CQuaternionExt::SummaryCB && m_StructValuePtr!=NULL )    // is tw_type_quat?
        {
            if( _Value!=NULL )
            {
                ...
                double x, y, z;
                if(SDL_sscanf(_Value, "%lf %lf %lf", &x, &y, &z)==3 )
                {
                    dir[0] = x; 
                    dir[1] = y;
                    dir[2] = z;
    ...
    case VG_ARROWCOLOR:
        if( m_SummaryCallback==CQuaternionExt::SummaryCB && m_StructValuePtr!=NULL )    // is tw_type_quat?
        {
            ...
            {
                int r, g, b;
                if(SDL_sscanf(_Value, "%d %d %d", &r, &g, &b)==3 )
...
int CTwBar::SetAttrib(int _AttribID, const char *_Value)
{
    ...
    case BAR_COLOR:
        ...
        {
            int v0, v1, v2, v3;
            int n = SDL_sscanf(_Value, "%d%d%d%d", &v0, &v1, &v2, &v3);
            color32 c;
    ...
    case BAR_ALPHA:
        ...
        {
            int alpha = 255;
            int n = SDL_sscanf(_Value, "%d", &alpha);
    ...
    case BAR_SIZE:
        ...
        {
            int sx, sy;
            int n = SDL_sscanf(_Value, "%d%d", &sx, &sy);
            if( n==2 && sx>0 && sy>0 )
            {
                m_Width = sx;
                m_Height = sy;
            ...
            else if( m_IsHelpBar )
            {
                ...
                else
                {
                    if( g_TwMgr->m_InfoBuildText )
                    {
                        string Info = "atb ";
                        char Ver[64];
						SDL_snprintf(Ver, 64, " %d.%02d", TW_VERSION/100, TW_VERSION%100);
                        Info += Ver;
    ...
    if( _Commit && m_EditInPlace.m_Active && m_EditInPlace.m_Var!=NULL )
    {
        ...
        else if( IsCSStringType(m_EditInPlace.m_Var->m_Type) )
        {
            ...
            if( n>0 )
            {
                ...
                {
                    if( n>1 )
						SDL_strlcpy((char *)m_EditInPlace.m_Var->m_Ptr, m_EditInPlace.m_String.c_str(), n-1);
        ...
        else
        {
			...
			if (m_EditInPlace.m_Var->m_Type == TW_TYPE_CHAR)
			{
				...
				n = SDL_sscanf(m_EditInPlace.m_String.c_str(), "%c", &Char);
				Val = Char;
			}
			else
			{
				float tempVal = 0;
                n = SDL_sscanf(m_EditInPlace.m_String.c_str(), "%f", &tempVal);
				Val = (double)tempVal;
			}
            if( n==1 )
            {
